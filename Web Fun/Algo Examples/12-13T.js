...
class MinHeap {
     ...
     */
    extract() {
        const extracted = this.heap[1];
        this.heap[1] = this.heap[this.heap.length-1];
        this.heap.pop();
        let idx = 1;
        while (true) {
            if (this.heap[this.idxOfLeftChild(idx)] < this.heap[this.idxOfRightChild(idx)]) {
                if (this.heap[this.idxOfLeftChild(idx)] < this.heap[idx]) {
                    this.swap(idx, this.idxOfLeftChild(idx));
                    idx = this.idxOfLeftChild(idx);
                } else {
                    return extracted;
                }
            } else {
                if (this.heap[this.idxOfRightChild(idx)] < this.heap[idx]) {
                    this.swap(idx, this.idxOfRightChild(idx));
                    idx = this.idxOfRightChild(idx);
                } else {
                    return extracted;
                }
            }
